...
class Camera(QtGui.QWidget):
    ...
    def start(self):
        ...
        cap_q, det_q, out_q, res_q = Queue.Queue(), Queue.Queue(), Queue.Queue(), Queue.Queue()
        if self.idx < 6: gpu = 0
        else: gpu = 1
        self.pool = [ATCCThread(out_q, res_q, self.params, self), DetectionThread(det_q, out_q, self.params, gpu),
                     ReaderThread(cap_q, self.params), PreprocThread(cap_q, det_q, self.params)]
    ...
    def getNextFrame(self, text):
        ...
        if state > 0:
            try:
                ret, frame = self.res_q.get(timeout=0.1)
            except:
                ret = False
