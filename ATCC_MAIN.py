...
class ATCCProc(Process):
    ...
    def __init__(self, transport, queue, showcase, link, daemon=False):
        Process.__init__(self)
        if '18.2' in link: self.link = 'ParkJL.mp4'
        else: self.link = 'MayoRd.mp4'
        self.transport = transport
...
class ATCCMainWindow(QtGui.QMainWindow):
    ...
    def checkLogin(self):
        cur = self.DBconn.cursor()
        cur.execute("select * from Machines where MacID = "+str(uuid.getnode())+";")
...
class Camera(QtGui.QWidget):
    ...
    def togglePlay(self):
        ...
        if state > 0:
            ...
            self.proc = ATCCProc(child_pipe, queue, self.isShowcasing, self.link)
            self.proc.start()
            self.emitter.start()
